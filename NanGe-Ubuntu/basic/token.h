...
class Token
{
 ...
 protected:
  ...
  public:
	...
	int isIdent() { return Type==IDENT; }
	int isIdent(const char *s) { return (Type==IDENT) && (!strcmp(s,Buffer)); }
	int isInteg() { return Type==INTEG; }
	int isInteg(int n) { return (Type==INTEG) && (atoi(Buffer)==n); }
	int isInteg(const char *s){return (Type==INTEG) && (atoi(Buffer)==atoi(s)); }
	int isDelim() { return Type==DELIM; }
	int isDelim(const char *s) { return (Type==DELIM) && (!strcmp(s,Buffer)); }
	int isEmpty() { return !*Buffer; }

	int isEqualTo(Token &t)
	{
		if (Type!=t.Type) return FALSE;
		else if (Type==INTEG) return atoi(Buffer)==atoi(t.Buffer);
		else return !strcmp(Buffer,t.Buffer);
	}
	int isEqualTo(const char *s)
	{
	...
	int isEqualTo(int n) { return isInteg(n); }

	int operator == ( Token &t ) { return isEqualTo(t); }
	int operator == ( const char *s) { return isEqualTo(s); }
	int operator == ( int n) { return isInteg(n); }
	int operator != ( Token &t ) { return (!isEqualTo(t)); }
	int operator != ( const char *s) { return (!isEqualTo(s)); }
	int operator != ( int n) { return (!isInteg(n)); }
